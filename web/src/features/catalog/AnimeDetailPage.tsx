...
export function AnimeDetailPage({ onPlayThemes, onPlayNext, onAddToQueue, onPlaySong, onPlayNextSong, onAddToQueueSong, onReplaceQueueSong }: AnimeDetailPageProps = {}) {
  ...
  const readyThemes = sortedThemes.filter((theme) => theme.audioState === 'READY')
  return (
    <section className="page catalog-page catalog-detail" aria-labelledby="anime-title">
      ...
      <section className="catalog-section" aria-labelledby="anime-themes-title"><div className="catalog-section__heading"><div><p className="eyebrow">Openings and endings</p><h2 id="anime-themes-title">Themes</h2></div><span className="catalog-section__count">{sortedThemes.length}</span></div>{sortedThemes.length === 0 ? <p className="catalog-empty">No themes are available for this anime yet.</p> : <div className="catalog-theme-list">{sortedThemes.map((theme) => { const startIndex = readyThemes.indexOf(theme); return <ThemeRow key={theme.id} theme={theme} onPlay={onPlayThemes && startIndex >= 0 ? () => onPlayThemes(readyThemes, startIndex, false, anime.posterUrl) : undefined} onMore={() => setActionThemes([theme])} /> })}</div>}</section>
      <section className="catalog-section" aria-labelledby="anime-music-title"><div className="catalog-section__heading"><div><p className="eyebrow">Ready catalog</p><h2 id="anime-music-title">Music releases</h2></div><Link className="catalog-section__link" to={`/anime/${encodeURIComponent(anime.kitsuId)}/related-music`}>See all</Link></div>{music.isError ? <CatalogError title="Music catalog unavailable" message="Themes are still available above. Try again later for album and track details." error={music.error} onRetry={() => void music.refetch()} /> : music.isPending ? <CatalogLoading label="Loading music releases" /> : music.data?.releases.length ? <div className="catalog-release-grid">{music.data.releases.map((release) => <article className="catalog-release-card" key={release.id}>{release.artworkUrl ? <img src={browserAssetUrl(release.artworkUrl)} alt="" loading="lazy" /> : <span className="catalog-release-card__art" aria-hidden="true"><Disc3 size={26} /></span>}<div><h3><Link to={`/release/${release.id}`}>{release.title}</Link></h3><p>{release.artistCredit || 'Various artists'}</p><small>{release.tracks.length} {release.tracks.length === 1 ? 'track' : 'tracks'}{release.year ? ` · ${release.year}` : ''}</small><ol className="catalog-release-tracks">{release.tracks.map((track) => <AnimeMusicTrackRow key={track.id} track={track} release={release} anime={anime} artworkUrl={release.artworkUrl} onPlay={onPlaySong} onPlayNext={onPlayNextSong} onAddToQueue={onAddToQueueSong} onReplaceQueue={onReplaceQueueSong} onNavigate={navigate} />)}</ol></div></article>)}</div> : <p className="catalog-empty">No ready music releases are available for this anime yet.</p>}</section>
...
function ThemeRow({ theme, onPlay, onMore }: { theme: LibraryThemeDto; onPlay?: () => void; onMore: () => void }) {
  return <article className="catalog-theme-row"><span className="catalog-theme-row__type">{formatThemeType(theme.themeType) ?? 'Theme'}</span><button type="button" className="catalog-theme-row__art" onClick={onPlay} disabled={!onPlay || theme.audioState !== 'READY'} aria-label={`Play ${theme.title}`}><Play size={18} fill="currentColor" /></button><div><h3>{theme.title}</h3><p>{theme.artists.map((artist) => artist.name).filter(Boolean).join(', ') || 'Unknown artist'}</p></div><span className="catalog-theme-row__state">{theme.audioState === 'READY' ? 'Ready' : theme.audioState.toLowerCase()}</span><button type="button" className="catalog-theme-row__more" onClick={onMore} aria-label={`More actions for ${theme.title}`}><MoreHorizontal size={18} /></button></article>
}

function AnimeMusicTrackRow({ track, release, anime, artworkUrl, onPlay, onPlayNext, onAddToQueue, onReplaceQueue, onNavigate }: {
  track: MusicTrackDto
