...
export function AnimeDetailPage({ onPlayThemes, onPlayNext, onAddToQueue, onPlaySong, onPlayNextSong, onAddToQueueSong, onReplaceQueueSong }: AnimeDetailPageProps = {}) {
  ...
  return (
    <section className="page catalog-page catalog-detail" aria-labelledby="anime-title">
      ...
      <section className="catalog-section" aria-labelledby="anime-themes-title"><div className="catalog-section__heading"><div><p className="eyebrow">Openings and endings</p><h2 id="anime-themes-title">Themes</h2></div><span className="catalog-section__count">{sortedThemes.length}</span></div>{sortedThemes.length === 0 ? <p className="catalog-empty">No themes are available for this anime yet.</p> : <div className="catalog-theme-list">{sortedThemes.map((theme, index) => <ThemeRow key={theme.id} theme={theme} animeTitle={title} artworkUrl={artworkUrl ?? null} onPlay={onPlayThemes ? () => onPlayThemes(sortedThemes, index, false, anime.posterUrl) : undefined} onMore={() => setActionThemes([theme])} />)}</div>}</section>
...
function ThemeRow({ theme, animeTitle, artworkUrl, onPlay, onMore }: { theme: LibraryThemeDto; animeTitle: string; artworkUrl: string | null; onPlay?: () => void; onMore: () => void }) {
  const typeLabel = formatThemeType(theme.themeType) ?? 'Theme'
  const songTitle = theme.title.trim() || 'Untitled theme'
  const artist = theme.artists.map((artist) => artist.name.trim()).filter(Boolean).join(', ')
  return <article className="catalog-theme-row catalog-theme-row--anime-detail"><button type="button" className="catalog-theme-row__play" onClick={onPlay} disabled={!onPlay} aria-label={`Play ${songTitle}`}><Play size={18} fill="currentColor" /></button><MediaArtwork className="catalog-theme-row__anime-artwork" imageUrl={artworkUrl} label={`${animeTitle} artwork unavailable`} fallback={<span aria-hidden="true">AO</span>} testId={`theme-anime-artwork-${theme.id}`} /><div className="catalog-theme-row__copy"><h3><span className="catalog-theme-row__type">{typeLabel}</span><span className="catalog-theme-row__anime-title" title={animeTitle}>{animeTitle}</span></h3><p>{artist ? `${songTitle} · ${artist}` : songTitle}</p></div><button type="button" className="catalog-theme-row__more" onClick={onMore} aria-label={`More actions for ${songTitle}`}><MoreHorizontal size={18} /></button></article>
}
