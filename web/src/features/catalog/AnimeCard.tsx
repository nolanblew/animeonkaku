...
export function AnimeCard({ anime, themeCount, onPlayAnime }: { anime: LibraryAnimeDto; themeCount?: number; onPlayAnime?: (anime: LibraryAnimeDto) => void }) {
    ...
    <article className="catalog-anime-card" data-testid="anime-card">
      <Link className="catalog-anime-card__link" to={`/anime/${encodeURIComponent(anime.kitsuId)}`} aria-label={title}>
        ...
        <div className="catalog-anime-card__body">
          ...
          <div className="catalog-anime-card__meta"><span>{statusLabel(anime.watchingStatus)}</span>{themeCount !== undefined && <span>{themeCount} {themeCount === 1 ? 'theme' : 'themes'}</span>}</div>
        </div>
      </Link>
      <button type="button" className="catalog-anime-card__play" onClick={() => onPlayAnime?.(anime)} disabled={!onPlayAnime} aria-label="Play anime"><Play size={18} fill="currentColor" aria-hidden="true" /><span>Play anime</span></button>
    </article>
  )
}
