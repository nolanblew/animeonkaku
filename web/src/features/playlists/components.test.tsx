...
describe('playlist components', () => {
  ...
  it('renders playlist detail as a resolved music page without inline item editing', async () => {
    ...
    const onBack = vi.fn()
    renderWithQuery(<PlaylistDetail playlist={playlist()} library={displayLibrary()} onUpdate={onUpdate} onDelete={onDelete} onBack={onBack} onPlay={onPlay} onPlayItem={onPlayItem} onPlayNextItem={onPlayNextItem} onAddToQueueItem={onAddToQueueItem} />)
    ...
    await userEvent.click(screen.getByRole('button', { name: 'More actions for Real Opening' }))
    expect(screen.getByRole('menu', { name: 'Real Opening actions' })).toBeInTheDocument()
