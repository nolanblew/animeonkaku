...
describe('expanded player viewport contract', () => {
  it('locks the route to the dynamic viewport and gives the queue one bounded internal scroller', () => {
    ...
    expect(playerCss).toMatch(/player-now-playing--queue-open[^}]*grid-template-columns:\s*minmax\([^;]+minmax\([^;]+minmax\(/)
    expect(playerCss).toMatch(/player-queue[^}]*position:\s*relative/)
    expect(playerCss).toMatch(/@media \(max-width:\s*1180px\)[\s\S]*player-queue[^}]*top:\s*auto[^}]*height:\s*min\(23rem,44vh\)/)
    expect(playerCss).toMatch(/@media \(max-width:\s*700px\)[\s\S]*player-queue[^}]*height:\s*min\(25rem,48vh\)/)
  })
})
