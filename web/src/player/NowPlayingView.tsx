import { Heart, Maximize, MoreHorizontal, Pause, Play, Repeat, Shuffle, SkipBack, SkipForward } from 'lucide-react'
import { useState } from 'react'
import { usePlayer } from './PlayerProvider'
import type { PlaybackMode } from '../media/modeSwitch'

export interface NowPlayingViewProps { className?: string }
    ...
    <section className={['player-now-playing', isVideo ? 'player-now-playing--video' : 'player-now-playing--song', className].filter(Boolean).join(' ')} aria-label="Now playing" data-testid="now-playing-view">
      ...
      <div className="player-now-playing__body">
        <div className="player-now-playing__stage">
          {!isVideo && <div className="player-now-playing__artwork-wrap">{current?.artworkUrl ? <img className="player-now-playing__artwork" src={current.artworkUrl} alt="" /> : <div className="player-now-playing__artwork player-now-playing__artwork--fallback" aria-hidden="true">AO</div>}</div>}
        ...
        <div className="player-now-playing__details">
          ...
          <div className="player-waveform" aria-hidden="true">{Array.from({ length: 56 }, (_, index) => <span key={index} style={{ height: `${18 + ((index * 17) % 70)}%` }} />)}</div>
