...
const logQuery = z.object({ level: z.enum(["INFO", "WARN", "ERROR"]).optional(), limit: z.coerce.number().int().min(1).max(500).default(200) });
...
function dashboardPage(settings: MusicSearchSettingsDto): string {
  ...
  :root{--accent:#b58cff;--good:#73dbab;--warn:#ffca6a;--bad:#ff8099}.layout{min-height:100vh;display:grid;grid-template-columns:245px 1fr}.sidebar{padding:24px 18px;border-right:1px solid var(--border);background:#0d0e17e8;position:sticky;top:0;height:100vh}.brand{display:flex;align-items:center;gap:12px;font-weight:850;padding:0 8px 24px}.brand-mark{width:39px;height:39px;border-radius:13px;background:linear-gradient(145deg,#ff8eb4,#9d7cff);display:grid;place-items:center}.nav{display:grid;gap:5px}.nav button{border:0;background:transparent;color:var(--muted);font:inherit;text-align:left;padding:11px 13px;border-radius:11px;cursor:pointer}.nav button:hover,.nav button.active{color:var(--text);background:#242238}.sidebar-foot{position:absolute;bottom:22px;left:18px;right:18px}.content{min-width:0;padding:30px clamp(18px,4vw,50px) 60px}.topbar{display:flex;justify-content:space-between;align-items:center;margin-bottom:28px}.topbar h1{margin:0;font-size:28px}.muted{color:var(--muted)}.section{display:none}.section.active{display:block}.metrics{display:grid;grid-template-columns:repeat(3,minmax(150px,1fr));gap:13px}.metric{padding:20px}.metric strong{font-size:28px;display:block;margin-top:5px}.storage{margin-top:18px;padding:22px}.storage-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.storage-item{padding:14px;border-radius:13px;background:#1b1c2a}.toolbar{display:flex;gap:10px;margin-bottom:15px}.search{flex:1;max-width:440px;background:#171827;border:1px solid var(--border);border-radius:12px;color:var(--text);padding:11px 13px;font:inherit}.table-wrap{overflow:auto;border:1px solid var(--border);border-radius:17px;background:#12131e}table{width:100%;border-collapse:collapse;min-width:720px}th,td{text-align:left;padding:13px 14px;border-bottom:1px solid #262737;font-size:13px}th{color:var(--muted);font-size:11px;text-transform:uppercase;letter-spacing:.08em}.badge{display:inline-block;padding:4px 8px;border-radius:999px;background:#29273c;color:#d8c8ff;font-size:11px}.mini{border:1px solid #3a3a50;background:#202131;color:var(--text);border-radius:9px;padding:7px 10px;cursor:pointer;font:inherit;font-size:12px}.mini.danger{color:#ffb2c0;border-color:#633845}.mini:hover{filter:brightness(1.16)}.actions{display:flex;gap:7px;flex-wrap:wrap}.settings-card{padding:22px}.settings-card label{display:block;padding:9px}.toast{position:fixed;right:25px;bottom:25px;background:#232338;border:1px solid #55516f;padding:13px 16px;border-radius:12px;opacity:0;transform:translateY(10px);transition:.2s;pointer-events:none}.toast.show{opacity:1;transform:none}@media(max-width:850px){.layout{grid-template-columns:1fr}.sidebar{height:auto;position:static;border-right:0;border-bottom:1px solid var(--border)}.nav{display:flex;overflow:auto}.nav button{white-space:nowrap}.sidebar-foot{display:none}.metrics{grid-template-columns:repeat(2,1fr)}.storage-grid{grid-template-columns:repeat(2,1fr)}}
  ...
  function tableSection(id,placeholder){return ''}function renderTable(id,rows,columns){const body=rows.map(row=>'<tr>'+columns.map(c=>'<td>'+c.render(row)+'</td>').join('')+'</tr>').join('');$('#'+id+' .table-wrap').innerHTML='<table><thead><tr>'+columns.map(c=>'<th>'+c.label+'</th>').join('')+'</tr></thead><tbody>'+body+'</tbody></table>'}
