...
export interface AdminDashboardApi {
  ...
  reimportAnimeFullSize(kitsuId: string): Promise<Record<string, unknown>>;
...
export function registerAdminRoutes(
  ...
  app.post("/api/v1/admin/anime/:id/full-size/reimport", { schema: { params: idParams }, preHandler: requireAdmin }, async (request, reply) => reply.code(202).send(await dashboard.reimportAnimeFullSize(request.params.id)));
