...
import { MusicRequestNotFoundError, MusicRequestNotMappedError, type MusicRequestService } from "./service.js";

const animeParams = z.object({ kitsuId: z.string().min(1).max(100) });
const requestParams = z.object({ requestId: z.string().uuid() });

export function registerMusicRequestRoutes(fastify: FastifyInstance, auth: AuthService, service: Pick<MusicRequestService, "trigger" | "get" | "latest">): void {
  ...
  app.post("/v1/anime/:kitsuId/music-requests", { schema: { params: animeParams }, preHandler: requireAuth }, async (request, reply) => {
    try {
      const result = await service.trigger(request.auth!.user.kitsuUserId, request.params.kitsuId, "DEBUG_USER");
